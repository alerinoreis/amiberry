...
static void pfield_set_linetoscr();
...
static void xlinecheck(unsigned int start, unsigned int end)
{
	...
	if (!ok) {
		write_log(_T("*** %d-%d (%dx%dx%d %d) %p\n"),
			...
			gfxvidinfo.drawbuffer.width,
			gfxvidinfo.drawbuffer.height,
...
static void reset_decision_table()
{
...
bool isnativevidbuf()
{
	if (gfxvidinfo.outbuffer == nullptr)
		return false;
...
static void reset_custom_limits()
{
...
static void set_blanking_limits()
{
...
static void pfield_erase_hborder_sprites()
{
...
static void pfield_erase_vborder_sprites()
{
...
{
	uae_u16* b = reinterpret_cast<uae_u16 *>(buf);
	unsigned int i;
	unsigned int rem = 0;
	...
	{
		uae_u32* b2 = reinterpret_cast<uae_u32 *>(&b[i]);
		*b2 = col;
	}
	if (rem)
		b[stop] = uae_u16(col);
...
{
	uae_u32* b = reinterpret_cast<uae_u32 *>(buf);
	unsigned int i;
	xcolnr col = getbgc(blank);
...
static void pfield_do_fill_line(int start, int stop, bool blank)
{
	...
	{
	case 2: fill_line_16(xlinebuffer, start, stop, blank);
		break;
	case 4: fill_line_32(xlinebuffer, start, stop, blank);
		break;
	}
...
static void fill_line2(int startpos, int len)
{
	...
	nrem = nints & 7;
	nints &= ~7;
	start = reinterpret_cast<int *>(static_cast<uae_u8*>(xlinebuffer) + (startpos << shift));
	val = getbgc(false);
...
{
	uae_u16* buf = reinterpret_cast<uae_u16 *>(xlinebuffer);
...
{
	uae_u32* buf = reinterpret_cast<uae_u32 *>(xlinebuffer);
...
static void init_ham_decoding()
{
	...
	else if (currprefs.chipset_mask & CSMASK_AGA)
	{
		...
		{ /* AGA mode HAM8 */
			while (unpainted_amiga-- > 0)
			{
				...
				switch (pv & 0x3)
				{
				case 0x0: ham_lastcolor = colors_for_drawing.color_regs_aga[pc] & 0xffffff;
					break;
				case 0x1: ham_lastcolor &= 0xFFFF03;
					ham_lastcolor |= (pw & 0xFC);
					break;
				case 0x2: ham_lastcolor &= 0x03FFFF;
					ham_lastcolor |= (pw & 0xFC) << 16;
					break;
				case 0x3: ham_lastcolor &= 0xFF03FF;
					ham_lastcolor |= (pw & 0xFC) << 8;
					break;
				}
			}
		}
		...
		{ /* AGA mode HAM6 */
			while (unpainted_amiga-- > 0)
			{
				...
				switch (pv & 0x30)
				{
				case 0x00: ham_lastcolor = colors_for_drawing.color_regs_aga[pv & 0x0f] & 0xffffff;
					break;
				case 0x10: ham_lastcolor &= 0xFFFF00;
					ham_lastcolor |= pc << 0;
					break;
				case 0x20: ham_lastcolor &= 0x00FFFF;
					ham_lastcolor |= pc << 16;
					break;
				case 0x30: ham_lastcolor &= 0xFF00FF;
					ham_lastcolor |= pc << 8;
					break;
				}
			}
	...
	else
	{
		...
		while (unpainted_amiga-- > 0)
		{
			...
			switch (pv & 0x30)
			{
			case 0x00: ham_lastcolor = colors_for_drawing.color_regs_ecs[pv] & 0xfff;
				break;
			case 0x10: ham_lastcolor &= 0xFF0;
				ham_lastcolor |= (pv & 0xF);
				break;
			case 0x20: ham_lastcolor &= 0x0FF;
				ham_lastcolor |= (pv & 0xF) << 8;
				break;
			case 0x30: ham_lastcolor &= 0xF0F;
				ham_lastcolor |= (pv & 0xF) << 4;
				break;
			}
		}
	}
...
static void clear_bitplane_border_aga()
{
...
STATIC_INLINE void pfield_doline_1(uae_u32* pixels, int wordcount, int planes)
{
	while (wordcount-- > 0)
	{
		...
		{
#ifdef AGA
		case 8: b0 = GETLONG(real_bplpt[7]);
			real_bplpt[7] += 4;
		case 7: b1 = GETLONG(real_bplpt[6]);
			real_bplpt[6] += 4;
#endif
		case 6: b2 = GETLONG(real_bplpt[5]);
			real_bplpt[5] += 4;
		case 5: b3 = GETLONG(real_bplpt[4]);
			real_bplpt[4] += 4;
		case 4: b4 = GETLONG(real_bplpt[3]);
			real_bplpt[3] += 4;
		case 3: b5 = GETLONG(real_bplpt[2]);
			real_bplpt[2] += 4;
		case 2: b6 = GETLONG(real_bplpt[1]);
			real_bplpt[1] += 4;
		case 1: b7 = GETLONG(real_bplpt[0]);
			real_bplpt[0] += 4;
		}
...
#define DATA_POINTER(n) ((debug_bpl_mask & (1 << n)) ? (line_data[lineno] + (n) * MAX_WORDS_PER_LINE * 2) : (debug_bpl_mask_one ? all_ones : all_zeros))
	...
	{
	default: break;
	case 0: memset(data, 0, wordcount * 32);
		break;
	case 1: pfield_doline_n1(data, wordcount);
		break;
	case 2: pfield_doline_n2(data, wordcount);
		break;
	case 3: pfield_doline_n3(data, wordcount);
		break;
	case 4: pfield_doline_n4(data, wordcount);
		break;
	case 5: pfield_doline_n5(data, wordcount);
		break;
	case 6: pfield_doline_n6(data, wordcount);
		break;
#ifdef AGA
	case 7: pfield_doline_n7(data, wordcount);
		break;
	case 8: pfield_doline_n8(data, wordcount);
		break;
#endif
	}
...
void init_row_map()
{
	...
	row_map_genlock_buffer = nullptr;
	...
	row_map_color_burst_buffer = nullptr;
...
static void pfield_draw_line(struct vidbuffer* vb, int lineno, int gfx_ypos, int follow_ypos)
{
	...
	if (xlinebuffer == nullptr && do_double
...
static void init_drawing_frame()
{
	...
	if (currprefs.gfx_resolution == changed_prefs.gfx_resolution && lines_count > 0)
	{
		...
		{
			//TODO:		if (largest_res == RES_SUPERHIRES && (gfxvidinfo.gfx_resolution_reserved < RES_SUPERHIRES || gfxvidinfo.gfx_vresolution_reserved < 1))
			//			{
							...
							if ((src->width > 0 && src->height > 0) || (currprefs.gfx_api || currprefs.gf[0].gfx_filter > 0))
							{
								...
								if (changed_prefs.gfx_resolution != nr || changed_prefs.gfx_vresolution != nl)
								{
									...
									write_log(_T("RES -> %d (%d) LINE -> %d (%d) (%d - %d, %d - %d)\n"),
										...
										nl,
										nl_o,
										currprefs.gfx_autoresolution_minh,
										currprefs.gfx_autoresolution_minv,
										gfxvidinfo.gfx_resolution_reserved,
...
void putpixel(uae_u8* buf, uae_u8* genlockbuf, int bpp, int x, xcolnr c8, int opaq)
{
	...
	case 2:
		{
			uae_u16* p = reinterpret_cast<uae_u16*>(buf) + x;
			*p = uae_u16(c8);
			break;
		}
	case 3:
	...
	case 4:
		{
			...
			{
				uae_u32* p = reinterpret_cast<uae_u32*>(buf) + x;
				*p = c8;
			}
			else
			{
...
static void draw_debug_status_line(int line)
{
	...
	if (xlinebuffer == 0)
		xlinebuffer = row_map[line];
	xlinebuffer_genlock = row_map_genlock[line];
	//TODO:	debug_draw(xlinebuffer, gfxvidinfo.drawbuffer.pixbytes, line, gfxvidinfo.drawbuffer.outwidth, gfxvidinfo.drawbuffer.outheight, xredcolors, xgreencolors, xbluecolors);
}
...
static void refresh_indicator_init()
{
	xfree(refresh_indicator_buffer);
	refresh_indicator_buffer = nullptr;
	xfree(refresh_indicator_changed);
	refresh_indicator_changed = nullptr;
	xfree(refresh_indicator_changed_prev);
	refresh_indicator_changed_prev = nullptr;
...
static void refresh_indicator_update(struct vidbuffer* vb)
{
	for (int i = 0; i < max_ypos_thisframe; i++)
	{
		...
		for (int x = 0; x < 8; x++)
		{
			putpixel(xlinebuffer, nullptr, gfxvidinfo.drawbuffer.pixbytes, x, xcolors[color1], 1);
		}
		for (int x = 8; x < 16; x++)
		{
			putpixel(xlinebuffer, nullptr, gfxvidinfo.drawbuffer.pixbytes, x, xcolors[color2], 1);
		}
	}
}
...
void vsync_handle_redraw(int long_field, int lof_changed, uae_u16 bplcon0p, uae_u16 bplcon3p)
{
	...
	if (lof_changed || interlace_seen <= 0 || (currprefs.gfx_iscanlines && interlace_seen > 0) || last_redraw_point >= 2 || long_field || doublescan < 0)
	{
		...
		if (quit_program < 0)
		{
			...
			if (!savestate_state)
			{
				//TODO:				if (currprefs.quitstatefile[0])
				//				{
...
void hsync_record_line_state(int lineno, enum nln_how how, int changed)
{
	...
	changed |= frame_redraw_necessary != 0 || refresh_indicator_buffer != nullptr ||
...
static void gen_direct_drawing_table()
{
#ifdef AGA
	// BYPASS color table
	for (int i = 0; i < 256; i++)
