...
void inputdevice_do_keyboard(int code, int state)
{
	...
	if (code < 0x80)
	{
		...
		else if ((keybuf[AK_CTRL] || keybuf[AK_RCTRL]) && keybuf[AK_LAMI] && keybuf[AK_RAMI])
		{
			...
			//TODO: send_internalevent(INTERNALEVENT_KBRESET);
			uae_reset(r, 1);
		}
		if (record_key((uae_u8)((key << 1) | (key >> 7))))
		{
			//TODO:			if (inputdevice_logging & 1)
	...
	case 4: /* ->Parallel port joystick adapter port #2 */
		...
		if (ie->type & 4)
		{
			...
			if (state)
			{
				...
				//TODO:					gui_gameport_button_change (joy, ie->data, 1);
			}
			else
			{
				joybutton[joy] &= ~(1 << ie->data);
				//TODO:					gui_gameport_button_change (joy, ie->data, 0);
			}

			//TODO:				if (ie->data == 0 && old != (joybutton[joy] & (1 << ie->data)) && currprefs.cpu_cycle_exact) {
...
int send_input_event(int nr, int state, int max, int autofire)
{
	check_enable(nr);
#ifdef INPUTDEVICE_SIMPLE
	return handle_input_event(nr, state, max, autofire);
#else
