	...
	if (!cputrace.readcounter && !cputrace.writecounter && !cputrace.cyclecounter) {
		if (cpu_tracer != -2) {
			write_log(_T("CPU trace: dma_cycle() enabled. %08x %08x NOW=%08lx\n"),
				cputrace.cyclecounter_pre,
				cputrace.cyclecounter_post,
				get_cycles());
...
static void build_cpufunctbl(void)
{
	...
	write_log(_T("Building CPU, %d opcodes (%d %d %d)\n"),
		opcnt,
		lvl,
		currprefs.cpu_cycle_exact ? -2 : currprefs.cpu_memory_cycle_exact ? -1 : currprefs.cpu_compatible ? 1 : 0,
...
static void update_68k_cycles(void)
{
	...
	if (currprefs.m68k_speed >= 0 && !currprefs.cpu_cycle_exact) {
		if (currprefs.m68k_speed_throttle < 0) {
			cycles_mult = static_cast<unsigned long>(CYCLES_DIV * 1000 / (1000 + currprefs.m68k_speed_throttle));
		}
...
static void m68k_run_jit(void)
{
	...
	for (;;) {
		static_cast<compiled_handler*>(pushall_call_handler)();
		/* Whenever we return from that, we should check spcflags */
		check_uae_int_request();
		if (regs.spcflags) {
			if (do_specialties(0)) {
