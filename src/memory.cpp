...
static unsigned int bootrom_filepos, chip_filepos, bogo_filepos, a3000lmem_filepos, a3000hmem_filepos, mem25bit_filepos;
...
int addr_valid(const TCHAR* txt, uaecptr addr, uae_u32 len)
{
	...
	if (ab == nullptr || !(ab->flags & (ABFLAG_RAM | ABFLAG_ROM)) || addr < 0x100 || len > 16777215 || !valid_address(addr, len))
	{
...
//			exception2(addr);
		//TODO:		exception2(addr, true, size, regs.s ? 4 : 0);		
	}
}
...
//		if (gary_toenb)
//			exception2(addr);
//TODO:			exception2(addr, false, size, (regs.s ? 4 : 0) | (inst ? 0 : 1));
		return v;
	}
...
static uae_u32 REGPARAM2 chipmem_lget_ce2(uaecptr addr)
{
	...
	m = reinterpret_cast<uae_u32 *>(chipmem_bank.baseaddr + addr);
	ce2_timeout();
...
static uae_u32 REGPARAM2 chipmem_wget_ce2(uaecptr addr)
{
	...
	addr &= chipmem_bank.mask;
	m = reinterpret_cast<uae_u16 *>(chipmem_bank.baseaddr + addr);
	ce2_timeout();
	v = do_get_mem_word(m);
	return v;
}
...
static void REGPARAM2 kickmem_lput(uaecptr addr, uae_u32 b)
{
	...
	if (currprefs.rom_readwrite && rom_write_enabled)
	{
		...
		m = reinterpret_cast<uae_u32 *>(kickmem_bank.baseaddr + addr);
		do_put_mem_long(m, b);
#if 0
...
static void REGPARAM2 kickmem_wput(uaecptr addr, uae_u32 b)
{
	...
	{
		addr &= kickmem_bank.mask;
		m = reinterpret_cast<uae_u16 *>(kickmem_bank.baseaddr + addr);
		do_put_mem_word(m, b);
	}
	else if (a1000_kickstart_mode)
	{
	...
	if (quit_program == 0)
	{
		...
		if ((currprefs.cpu_model > 68000 || !currprefs.cpu_compatible) && !currprefs.mmu_model)
		{
			...
			if (be_cnt < 3)
			{
				...
				//TODO:				if (debugging || DEBUG_STUPID)
				//				{
				//					activate_debugger();
				//					m68k_dumpstate(0);
				//				}
...
addrbank chipmem_dummy_bank = {
	...
	nullptr,
	nullptr,
	_T("Dummy Chip memory"),
	dummy_lgeti,
	dummy_wgeti,
	ABFLAG_IO | ABFLAG_CHIPRAM,
	S_READ,
	S_WRITE
};
...
addrbank chipmem_bank_ce2 = {
	...
	nullptr,
	nullptr,
	_T("Chip memory (68020 'ce')"),
	chipmem_lget_ce2,
	chipmem_wget_ce2,
	ABFLAG_RAM | ABFLAG_CHIPRAM,
	S_READ,
	S_WRITE
};
#endif
...
addrbank kickram_bank = {
	...
	nullptr,
	nullptr,
	_T("Kickstart Shadow RAM"),
	kickmem_lget,
	kickmem_wget,
	ABFLAG_UNK | ABFLAG_SAFE,
	0,
	S_WRITE
};

addrbank extendedkickmem_bank = {
	extendedkickmem_lget,
	extendedkickmem_wget,
	extendedkickmem_bget,
	extendedkickmem_lput,
	extendedkickmem_wput,
	extendedkickmem_bput,
	extendedkickmem_xlate,
	extendedkickmem_check,
	nullptr,
	nullptr,
	_T("Extended Kickstart ROM"),
	extendedkickmem_lget,
	extendedkickmem_wget,
	ABFLAG_ROM | ABFLAG_THREADSAFE,
	0,
...
addrbank extendedkickmem2_bank = {
	extendedkickmem2_lget,
	extendedkickmem2_wget,
	extendedkickmem2_bget,
	extendedkickmem2_lput,
	extendedkickmem2_wput,
	extendedkickmem2_bput,
	extendedkickmem2_xlate,
	extendedkickmem2_check,
	nullptr,
	_T("rom_a8"),
	_T("Extended 2nd Kickstart ROM"),
	extendedkickmem2_lget,
	extendedkickmem2_wget,
	ABFLAG_ROM | ABFLAG_THREADSAFE,
	0,
...
{
	int i, j, patched = 0;
	const uae_char* residents[] = {"NCR scsi.device", nullptr};
...
bool mapped_malloc(addrbank* ab)
{
	...
	ab->allocated_size = ab->baseaddr != nullptr ? ab->reserved_size : 0;
...
static void allocate_memory()
{
		...
		(currprefs.chipset_mask & CSMASK_ECS_AGNUS) && !(currprefs.chipset_mask & CSMASK_AGA) && currprefs.cpu_model < 68020)
	{
		if ((chipmem_bank.reserved_size != currprefs.chipmem_size || bogomem_bank.reserved_size != currprefs.bogomem_size))
		{
			...
			if (chipmem_bank.baseaddr == nullptr)
			{
...
void map_overlay(int chip)
{
	...
	fill_ce_banks();
	//TODO:	cpuboard_overlay_override();
	...
	for (int i = 0; i < 2; i++)
	{
		if (currprefs.custom_memory_sizes[i])
		{
			map_banks(i == 0 ? &custmem1_bank : &custmem2_bank,
			          ...
			          currprefs.custom_memory_sizes[i] >> 16,
			          0);
...
void memory_init()
{
	...
	chipmem_bank.reserved_size = 0;
	bogomem_bank.reserved_size = 0;
	kickmem_bank.baseaddr = nullptr;
	extendedkickmem_bank.baseaddr = nullptr;
	extendedkickmem_bank.reserved_size = 0;
	extendedkickmem2_bank.baseaddr = nullptr;
	extendedkickmem2_bank.reserved_size = 0;
	extendedkickmem_type = 0;
	chipmem_bank.baseaddr = nullptr;
	mem25bit_bank.reserved_size = mem25bit_bank.reserved_size = 0;
	a3000lmem_bank.reserved_size = a3000hmem_bank.reserved_size = 0;
	a3000lmem_bank.baseaddr = a3000hmem_bank.baseaddr = nullptr;
	bogomem_bank.baseaddr = nullptr;
	cardmem_bank.baseaddr = nullptr;
	custmem1_bank.reserved_size = custmem2_bank.reserved_size = 0;
	custmem1_bank.baseaddr = nullptr;
	custmem2_bank.baseaddr = nullptr;
...
void memory_cleanup()
{
	...
	bogomem_bank.baseaddr = nullptr;
	kickmem_bank.baseaddr = nullptr;
	mem25bit_bank.baseaddr = nullptr;
	a3000lmem_bank.baseaddr = a3000hmem_bank.baseaddr = nullptr;
	a1000_bootrom = nullptr;
	a1000_kickstart_mode = 0;
	chipmem_bank.baseaddr = nullptr;
