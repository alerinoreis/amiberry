...
static void addop_ce020(instr *curi, int subhead)
{
	...
	int c = curi->clocks;
#if 0
	if ((((curi->sduse & 2) && !isreg(amodes(curi->smode))) || (((curi->sduse >> 4) & 2) && !isreg(curi->dmode))) && using_waitstates) {
		t += using_waitstates;
		c += using_waitstates;
	}
#endif
...
static void genamodedual(instr *curi,
	amodes smode,
	char *sreg,
	wordsizes ssize,
	char *sname,
...
static void gen_opcode(unsigned long int opcode)
{
	...
	case i_EOR:
		{
			...
			int c = 0;
			genamodedual(curi, amodes(curi->smode), "srcreg", wordsizes(curi->size), "src", 1, 0, amodes(curi->dmode), "dstreg", wordsizes(curi->size), "dst", 1, GF_RMW);
	...
	case i_SUBA:
		{
			int c = 0;
			genamodedual(curi, amodes(curi->smode), "srcreg", wordsizes(curi->size), "src", 1, 0, amodes(curi->dmode), "dstreg", sz_long, "dst", 1, GF_RMW);
		//genamode (curi, amodes(curi->smode), "srcreg", wordsizes(curi->size), "src", 1, 0, 0);
	...
	case i_TST:
		...
		break;
	case i_BTST:
		genamodedual(curi, amodes(curi->smode), "srcreg", wordsizes(curi->size), "src", 1, 0, amodes(curi->dmode), "dstreg", wordsizes(curi->size), "dst", 1, 0);
		//genamode (curi, amodes(curi->smode), "srcreg", wordsizes(curi->size), "src", 1, 0, 0);
	...
	case i_EXT:
		...
		genflags(flag_logical,
			curi->size == sz_word ? sz_word : sz_long,
			"dst",
			"",
			"");
		genastore("dst",
	...
	case i_DBcc:
		...
		genamodedual(curi,
			...
			GF_AA | GF_NOREFILL);
		//genamode (curi, amodes(curi->smode), "srcreg", wordsizes(curi->size), "src", 1, 0, GF_AA | GF_NOREFILL);
		//genamode (curi, amodes(curi->dmode), "dstreg", wordsizes(curi->size), "offs", 1, 0, GF_AA | GF_NOREFILL);
		printf("\tuaecptr oldpc = %s;\n", getpc);
		addcycles000(2);
		printf("\tif (!cctrue (%d)) {\n", curi->cc);
