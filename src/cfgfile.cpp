...
static unsigned int cfg_write(const void* b, struct zfile* z)
{
	unsigned int v;
	if (unicode_config)
	{
...
void cfgfile_backup(const TCHAR* path)
{
	...
	//TODO: bool hidden = my_isfilehidden(dpath);
	my_unlink(dpath);
...
int parse_cmdline_option(struct uae_prefs* p, TCHAR c, const TCHAR* arg)
{
	...
	case 'K': cmdpath(p->romextfile, arg, 255);
		break;
	case 'p': _tcsncpy(p->prtname, arg, 255);
		p->prtname[255] = 0;
		break;
...
static int bip_a4000(struct uae_prefs* p, int config, int compa, int romcheck)
{
	...
	case 2:
		...
		p->fpu_model = 68060;
		p->ppc_mode = 1;
		//TODO: cpuboard_setboard(p, BOARD_CYBERSTORM, BOARD_CYBERSTORM_SUB_PPC);
